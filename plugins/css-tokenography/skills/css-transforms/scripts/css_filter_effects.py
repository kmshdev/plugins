#!/usr/bin/env python3
"""Canonical typed CSS filter CLI."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate a typed ordered CSS filter list.")
    ...
    try:
        ...
        if data.get("property") != "filter": raise FilterInputError("property must be 'filter' for this CLI")
        report = build_filter_report(data)
    except FilterInputError as error:
        print(f"css-filter-effects: {error}", file=sys.stderr); return 1
